...
1. **配置加载** — 读取或创建 [config.json](config.json)（首次运行进入交互式引导）
...
1. 使用 Read 工具检查当前工作目录下是否存在 [config.json](config.json)
2. 若已存在 → 读取并解析配置值
3. 若不存在 → 进入**交互式配置引导**（仅首次运行）：
   ...
   i. 根据用户选择生成 [config.json](config.json)（JSON 格式）写入项目根目录
4. 解析以下配置值：
   - **视频模型**：用于生成视频的 AI 模型
...
1. **`args` 为 `"config"`**：使用 Read 工具打开当前目录下的 [config.json](config.json)，展示给用户，询问是否需要编辑。
