...
3. 若不存在 → 进入**交互式配置引导**（仅首次运行）：
   ...
   c. 询问语言，提供选项：auto / zh / en
   d. 询问每集分镜数（默认 15，建议 10-20）
   e. 询问默认模式：review / fast
   f. 告知用户其余配置使用默认值（每集时长目标: 1-2分钟、单镜头时长范围: 5-10秒、上下文集数: 3），可通过 `/short-video config` 随时修改
   g. 根据用户选择生成 `config.md` 写入项目根目录
4. 解析以下配置值：
   - **视频模型**：用于生成视频的 AI 模型
...
3. 展示 3 个选项给用户，等待用户选择
4. 将用户选择的主题作为故事输入，继续 Step 1
