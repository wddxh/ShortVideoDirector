---
description: 将故事创意转化为单集短视频的分镜提示词和资产图像提示词。输入故事点子或概述，输出完整的剧本、分镜和资产提示词。使用 /short-video
  启动，/short-video config 编辑配置。
agent: build
subtask: true
...
调用 task 工具，传入 agent: `director`，prompt: "执行 short-input-confirm skill 描述的任务，参数："{story_input}""
   - **无故事材料** → 调用 `short-plot-options` ，等待用户选择：

调用 task 工具，传入 agent: `director`，prompt: "执行 short-plot-options skill 描述的任务，无额外参数"
...
调用 task 工具，传入 agent: `director`，prompt: "执行 short-outline skill 描述的任务，参数："{用户确认的剧情方向文本}""

**4.2 Scriptwriter — 写剧本：**

调用 `scriptwriter-script`：

调用 task 工具，传入 agent: `scriptwriter`，prompt: "执行 scriptwriter-script skill 描述的任务，参数：ep01"

**4.3 Director — 审核剧本：**

1. 调用 `director-review-script`：

调用 task 工具，传入 agent: `director`，prompt: "执行 director-review-script skill 描述的任务，参数：ep01"
2. 若"需修改"→ 调用 `scriptwriter-fix-script`（最多 2 轮）：

调用 task 工具，传入 agent: `scriptwriter`，prompt: "执行 scriptwriter-fix-script skill 描述的任务，参数：ep01 "{修改意见}""
...
调用 task 工具，传入 agent: `storyboarder`，prompt: "执行 short-storyboard skill 描述的任务，参数：ep01"
...
调用 task 工具，传入 agent: `director`，prompt: "执行 short-review-storyboard skill 描述的任务，参数：ep01"
2. 若"需修改"→ 调用 `short-fix-storyboard`（最多 2 轮）：

调用 task 工具，传入 agent: `storyboarder`，prompt: "执行 short-fix-storyboard skill 描述的任务，参数：ep01 "{修改意见}""
