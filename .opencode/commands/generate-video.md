---
description: 将分镜提示词提交为视频生成任务。读取分镜和资产图片，提交到即梦CLI，异步跟踪任务状态。使用 /generate-video ep01 提交整集，或
  /generate-video ep01 镜头3 镜头5 提交指定镜头。
agent: build
subtask: true
...
调用 task 工具，传入 agent: `creator`，prompt: "执行 creator-video-dreamina skill 描述的任务，参数：{集数} {目标 pending 镜头编号列表，如 "1 3 5" 或 "all"}"

### 阶段 6: 启动自动监控

1. 调用 `auto-video`：

使用 skill 工具加载 `auto-video` skill 的内容并按其描述执行，参数：{集数} 1200
