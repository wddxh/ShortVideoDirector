---
description: 将故事创意转化为AI视频分镜提示词和资产图像提示词。支持持续创作，自动检测新故事/续写模式。输入故事点子、原文或概述，输出完整的分镜和资产提示词。使用
  /series-video 启动，/series-video config 编辑配置。
agent: build
subtask: true
...
**参数提取规则：**
1. 若 `$1`（第一个空格分割的词）为纯数字 → 解析为**总集数**，`$2` 及之后的内容作为故事材料候选
...
使用 skill 工具加载 `new-story` skill 的内容并按其描述执行，参数：{work_mode} {total_episodes} "{story_input}"
- **续写模式** → 调用 `continue-story`：

使用 skill 工具加载 `continue-story` skill 的内容并按其描述执行，参数：{work_mode} {total_episodes} "{story_input}"

参数说明：
- `$1` = work_mode（default / full-auto）
- `$2` = total_episodes（总集数，无则传空字符串）
- `$3` = story_input（故事材料，引号包裹，无则传空字符串）
