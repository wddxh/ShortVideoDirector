...
创作材料使用所选 provider 自有 material tool，具体命令、pack、token 计数/绑定和重基规则见该 provider 文档；Dreamina 见 skills/creator-provider-dreamina/video.md。草稿可供材料解析但不表示就绪。共享 assembler 仅提供无 provider token 的内部数据，不是公开通用 adapter；未来 provider 自行实现工具，无需 registry/framework/manifest schema 变更。共同风格在每个 task 的最终 prompt 内表达一次，不是整集只写一稿；适用共同事实可准确重复，不禁必要相同用词。不同基线交 owner，源事实及对白完整保留。资产图按成员首次使用求并集在前，本地媒体在后，每镜链接须自身 header 声明，sources 不上传。
