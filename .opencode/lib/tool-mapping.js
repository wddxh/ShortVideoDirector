...
素材时窗与用途按 skills/_meta/rules/visual-prompt-craft-common.md#参考区间与采用维度 同步到 use/最终 prompt，保留完整源动作、情绪意图及代理归属，局部动作不扩成全片循环。上传清除内部轨迹线、坐标、camera cone 和调试标签，保留正式 film text；抽象分工可说明省略，不能反转矛盾媒体事实。转场过程见 skills/_meta/rules/transition-craft.md#转场过程表达，沿源切点与预算，不加示例秒数或 extend。
