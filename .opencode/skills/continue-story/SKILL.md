...
调用 task 工具，传入 agent: `director`，prompt: "执行 director-input-confirm skill 描述的任务，参数："$3""
2. 展示说明给用户：
   - **A. 确认** — 继续阶段 3
   - **B. 重新生成** — 重新调用 `director-input-confirm` skill，传递参数：`"$3"`
   - **C. 补充说明** — 收集用户反馈，重新调用 `director-input-confirm` skill，传递参数：`"{用户反馈内容}"`
   - **[default mode 下等待用户确认；full-auto mode 下 Director 自动确认]**
3. 用户选择 A → 继续阶段 3

### 阶段 2.5: 生成剧情弧线（仅当 `$2` 非空且 story/arc.md 不存在时执行）

1. 调用 `director-arc`：

调用 task 工具，传入 agent: `director`，prompt: "执行 director-arc skill 描述的任务，参数：$2 "{选定的剧情方向}""

### 阶段 3: Director 生成新集大纲

1. 调用 `director-outline`：

调用 task 工具，传入 agent: `director`，prompt: "执行 director-outline skill 描述的任务，参数：ep{N+1} "{选定的剧情方向}""
...
调用 task 工具，传入 agent: `writer`，prompt: "执行 writer-novel skill 描述的任务，参数：ep{N+1}"
...
调用 task 工具，传入 agent: `writer`，prompt: "执行 writer-fix-novel skill 描述的任务，参数：ep{N+1} "当前字数为{实际字数}，目标范围为{下限}-{上限}，请调整内容使字数符合要求""

**4.2 Director — 审核小说原文：**

1. 调用 `director-review-novel`：

调用 task 工具，传入 agent: `director`，prompt: "执行 director-review-novel skill 描述的任务，参数：ep{N+1}"
2. 若"需修改"→ 调用 `writer-fix-novel`（最多 2 轮）：

调用 task 工具，传入 agent: `writer`，prompt: "执行 writer-fix-novel skill 描述的任务，参数：ep{N+1} "{修改意见}""
...
调用 task 工具，传入 agent: `storyboarder`，prompt: "执行 storyboarder-asset-list skill 描述的任务，参数：ep{N+1}"
...
调用 task 工具，传入 agent: `creator`，prompt: "执行 creator-create-assets skill 描述的任务，参数：ep{N+1}"
- 调用 `creator-update-records`：

调用 task 工具，传入 agent: `creator`，prompt: "执行 creator-update-records skill 描述的任务，参数：ep{N+1}"
...
调用 task 工具，传入 agent: `creator`，prompt: "执行 creator-generate-images skill 描述的任务，参数：ep{N+1}"

**分镜流程线（前台，正常推进）：**
1. 调用 `storyboarder-storyboard`：

调用 task 工具，传入 agent: `storyboarder`，prompt: "执行 storyboarder-storyboard skill 描述的任务，参数：ep{N+1}"
...
调用 task 工具，传入 agent: `director`，prompt: "执行 director-review-storyboard skill 描述的任务，参数：ep{N+1}"
2. 若"需修改"→ 调用 `storyboarder-fix-storyboard`（最多 2 轮）：

调用 task 工具，传入 agent: `storyboarder`，prompt: "执行 storyboarder-fix-storyboard skill 描述的任务，参数：ep{N+1} "{修改意见}""
