...
const FIXTURE = path.join(__dirname, 'fixtures/agents/reviewer.md');
...
describe('convertAgentFrontmatter', () => {
  test('reviewer fixture enables Task in both tools and permissions', async () => {
    const { frontmatter } = await parseAgentFile(FIXTURE);
    const out = convertAgentFrontmatter(frontmatter);
    assert.deepEqual(out.tools, { bash: true, skill: true, task: true });
    assert.equal(buildPermissionForAgent(frontmatter.name).task, 'allow');
    assert.throws(() => buildPermissionForAgent('director'), /Unknown agent: director/);
  });
  ...
  test('drops CC tools comma-string but explicitly enables bash tool', () => {
    ...
    const out = convertAgentFrontmatter({
      name: 'reviewer', description: 'Reviewer', tools: 'Read, Write', model: 'inherit'
    });
...
describe('buildPermissionForAgent', () => {
  ...
  test('all agents inherit BASE_PERMISSION defaults', () => {
    for (const agent of ['reviewer', 'scriptwriter', 'storyboarder', 'creator']) {
      ...
      assert.equal(p.task, ['reviewer', 'creator'].includes(agent) ? 'allow' : 'deny', `${agent}.task`);
