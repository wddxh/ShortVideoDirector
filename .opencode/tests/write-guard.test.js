...
describe('findLargeStrings', () => {
  ...
  test('returns entry for single long string', () => {
    const result = findLargeStrings({content: 'x'.repeat(3000)});
    assert.equal(result.length, 1);
    assert.equal(result[0].path, 'content');
    assert.equal(result[0].length, 3000);
  });

  test('reports nested object path', () => {
    const result = findLargeStrings({nested: {deep: {value: 'x'.repeat(3000)}}});
    assert.equal(result.length, 1);
    assert.equal(result[0].path, 'nested.deep.value');
  });

  test('reports array index path', () => {
    const result = findLargeStrings({items: ['small', 'x'.repeat(3000)]});
    assert.equal(result.length, 1);
    assert.equal(result[0].path, 'items[1]');
  });

  test('returns multiple if multiple long strings', () => {
    const result = findLargeStrings({a: 'x'.repeat(3000), b: 'y'.repeat(3000)});
...
describe('interceptToolCall', () => {
  ...
  test('throws on large content for write (regardless of file ext)', () => {
    assert.throws(
      () => interceptToolCall({tool: 'write'}, {args: {filePath: 'a.md', content: 'x'.repeat(3000)}}),
      /Tool 'write' has string argument\(s\) exceeding 2000 chars/
    );
    assert.throws(
      () => interceptToolCall({tool: 'write'}, {args: {filePath: 'data.json', content: 'x'.repeat(3000)}}),
      /exceeding 2000 chars/
    );
    assert.throws(
      () => interceptToolCall({tool: 'write'}, {args: {filePath: 'cfg.yaml', content: 'x'.repeat(3000)}}),
      /exceeding 2000 chars/
    );
  });
  ...
  test('throws on large newString for edit', () => {
    assert.throws(
      () => interceptToolCall({tool: 'edit'}, {args: {filePath: 'a.md', newString: 'x'.repeat(3000)}}),
      /Tool 'edit'.*newString: 3000 chars/
    );
  });

  test('throws on large prompt for task', () => {
    assert.throws(
      () => interceptToolCall({tool: 'task'}, {args: {prompt: 'x'.repeat(3000)}}),
      /Tool 'task'.*prompt: 3000 chars/
    );
  });

  test('throws on large patch for apply_patch', () => {
    assert.throws(
      () => interceptToolCall({tool: 'apply_patch'}, {args: {patch: 'x'.repeat(3000)}}),
      /Tool 'apply_patch'.*patch: 3000 chars/
    );
  });

  test('throws on large command for bash', () => {
    assert.throws(
      () => interceptToolCall({tool: 'bash'}, {args: {command: 'x'.repeat(3000)}}),
      /Tool 'bash'.*command: 3000 chars/
    );
  });

  test('error includes nested path', () => {
    assert.throws(
      () => interceptToolCall({tool: 'custom'}, {args: {nested: {deep: {value: 'x'.repeat(3000)}}}}),
      /nested\.deep\.value: 3000 chars/
    );
  });
