...
6. 若用户选择 A/B/C → 将用户选择的主题作为故事输入，继续 Step 3
...
1. 使用 Read 读取 [agents/director.md](../agents/director.md)
2. 使用 **Agent tool** 调用 Director 子代理，指令：基于用户提供的故事输入，生成一份结构化说明（含主题名称、核心设定、开篇钩子、卖点分析），格式与 Step 2a 的选项一致
