...
使用 Skill tool 调用 `new-story` skill，传递参数：`{work_mode} {total_episodes} "{story_input}"`
- **续写模式** → 调用 `continue-story`：

使用 Skill tool 调用 `continue-story` skill，传递参数：`{work_mode} {total_episodes} "{story_input}"`
