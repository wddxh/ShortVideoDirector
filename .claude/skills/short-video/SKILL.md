...
使用 Skill tool 调用 `short-input-confirm` skill，传递参数：`"{story_input}"`
   - **无故事材料** → 调用 `short-plot-options` ，等待用户选择：

使用 Skill tool 调用 `short-plot-options` skill（无参数）
...
使用 Skill tool 调用 `director-review-script` skill，传递参数：`ep01`
2. 若"需修改"→ 调用 `scriptwriter-fix-script`（最多 2 轮）：

使用 Skill tool 调用 `scriptwriter-fix-script` skill，传递参数：`ep01 "{修改意见}"`
...
使用 Skill tool 调用 `short-review-storyboard` skill，传递参数：`ep01`
2. 若"需修改"→ 调用 `short-fix-storyboard`（最多 2 轮）：

使用 Skill tool 调用 `short-fix-storyboard` skill，传递参数：`ep01 "{修改意见}"`
