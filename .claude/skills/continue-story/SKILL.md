...
使用 Skill tool 调用 `director-input-confirm` skill，传递参数：`"$ARGUMENTS[2]"`
2. 展示说明给用户：
   - **A. 确认** — 继续阶段 3
...
使用 Skill tool 调用 `director-arc` skill，传递参数：`$ARGUMENTS[1] "{选定的剧情方向}"`

### 阶段 3: Director 生成新集大纲

1. 调用 `director-outline`：

使用 Skill tool 调用 `director-outline` skill，传递参数：`ep{N+1} "{选定的剧情方向}"`
...
使用 Skill tool 调用 `writer-novel` skill，传递参数：`ep{N+1}`
...
使用 Skill tool 调用 `writer-fix-novel` skill，传递参数：`ep{N+1} "当前字数为{实际字数}，目标范围为{下限}-{上限}，请调整内容使字数符合要求"`

**4.2 Director — 审核小说原文：**

1. 调用 `director-review-novel`：

使用 Skill tool 调用 `director-review-novel` skill，传递参数：`ep{N+1}`
2. 若"需修改"→ 调用 `writer-fix-novel`（最多 2 轮）：

使用 Skill tool 调用 `writer-fix-novel` skill，传递参数：`ep{N+1} "{修改意见}"`
...
使用 Skill tool 调用 `storyboarder-asset-list` skill，传递参数：`ep{N+1}`
...
使用 Skill tool 调用 `director-review-storyboard` skill，传递参数：`ep{N+1}`
2. 若"需修改"→ 调用 `storyboarder-fix-storyboard`（最多 2 轮）：

使用 Skill tool 调用 `storyboarder-fix-storyboard` skill，传递参数：`ep{N+1} "{修改意见}"`
