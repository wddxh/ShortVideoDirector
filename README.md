...
- **caption review MP4**：从对应 clean 视频派生，带对白／旁白、时间码和内部切点标记；picture 宽高、帧率、时长与切点和 clean 一致，字幕放在新增底栏，总高度另加底栏高度，仅用于内部审阅。

正式 clean 默认与已选视频的比例、分辨率对齐，也可按你的明确要求采用本地参考规格。Creator 核实实际像素尺寸：16:9 + 720p 常见为 1280×720，只是示例，其他比例或 provider 档位需核实，不能按短边猜测。每集统一恒定帧率（CFR）：你已指定则采用，否则 Creator 为本集选择一次与源时钟兼容的 fps；未选择时不默认 30。

最迟在本集最早正式参考制作前，Director 协调配置 owner 保存实际值和来源（final settings 或用户 explicit local override）。例如以下仅为格式示例，值不是默认：
...
每集使用自己的 `epNN` 限定键。源码和导出读取已存规格；迭代草稿可以低清，正式选中的 clean 按保存值交付。先检查可复用媒体，合适就保留，不符项局部补导出或转换，保持相机、比例和源时钟，不能仅改 metadata 冒充匹配。Creator 交付前核实际参数，独立 Reviewer 采集配置和选中媒体后复核；工具诊断不代替语义审核。配置补齐与共享环境报告分开，不新增初始化探针或自动迁移历史；已有审核按改动范围评估兼容性，不盲刷指纹。
...
整集汇总读取同一已存规格，每组显式选中的完整 clean 都须匹配本集宽高／fps 及该组源时长；不符项先修正，工具不以最大画布自动补边掩盖错误。整集字幕 picture 保持 clean 尺寸，总高另加底栏；保留 canonical 全片时钟与已有音频，混合有声／无声时为无声段补等时静音，全无声保持无音轨。补充短参考按自身用途使用，不要求每条都覆盖全组。
