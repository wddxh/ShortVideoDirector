...
- 按 series.md / short.md 指引选取有关的剧本、规划或用户素材；缺少 arc 不妨碍有充分依据的单集规划
...
配置缺失、空白、读取失败、边界不清或冲突时由顶层主 AI/Director 直接澄清，不回退模板默认或其他 config.md；已有明确决定直接复用。系列保持初始共同目标，不以前集实际时长重设。先保存场景草稿再实跑，记录 sum、边界及退出状态。FAIL 先区分低于下限、超过上限与字段缺失/解析失败；后者先修计算依据，前两者按 rules.md 时长规划原则在授权内调整后复查，不一律删减。旧分配不足但净增后仍在原始确认区间内时更新场景目标；无法满足则报告冲突或询问用户，不自动扩大集边界。此校验适用于已采用的大纲，不要求为剧本制作补建大纲。
