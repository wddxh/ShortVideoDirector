...
小说是可选创作材料，仅在被委托时审核，不要求生产前补建小说/大纲。遵守共享规则的独立新 Reviewer context/主 AI relay；只写 `reviews/{ep}/novel.md` 的 Markdown 意见与轮次，不增加 runtime kind/evidence，不调度修复或记录用户批准。读取失败/不可判定返回 unknown，结局按用户意图判断，不强制闭环或悬念。
...
审核结果写入 `reviews/{ep}/novel.md`（append 模式，同一目标重审串行）。

**Round 自检**：
1. Read `reviews/{ep}/novel.md`（若不存在，本次为第 1 轮；否则按最大标题轮号递增）
...
### 文件操作
- 使用 Write 或 Edit 维护 `reviews/{ep}/novel.md`（append 模式，详见上文「输出格式」段的 Round 自检流程）
