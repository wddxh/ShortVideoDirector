...
输入可以是 `reviews/{ep}/assets/{category}/{name}.asset-visual.md` 中该目标当前 `{asset_path}|{image_path}` dirty entry，也可以是明确的 ep、基础资产路径与授权指令。引用审核时用 `review-evidence.mjs path asset-visual EP TARGET` 解析各文件，只读所属目标最新轮并核对当前输入；缺失或未完成只影响该目标。直接指令不要求旧 review；诊断委托不授权付费重生。
...
重生前先回查争议要求的来源及本图实际用途：用户明确要求、叙事/身份必需特征与自选可替换细节分别判断。局部精度问题只有影响真实需要才支持替换；owner 可在授权内简化或重设计并同步卡片，不靠反复采样追逐无作用的细节。核对相关折叠/展开、持有/位置与行动后果；需扩展目标、事实或写入路径时先通知原委托方并协调，持续有效 grants 内的必要修复不重复求许可。

左右或换手疑点先核对本图观察面、身体朝向和实际镜面/翻转依据，不能从画面左右直接推出角色自己的左右。必要视觉依据仍不明确时，保留旧图与不确定结论，交 Director 安排 fresh 独立视觉检查后再决定替换；不靠新提示或重生结果验证旧图是否有错。
...
资产参考诊断读取目标「资产参考」声明的必要直接卡和 PNG（可跨类别），按 prose 关系/用途核对正确来源、观察面和实际采用的外观/结构。包含另一资产、持有/穿戴与共享特征不一律按同实体处理；确属同实体才比较共同标志物、几何、材质与状态，判断现图复用或必要独立视图，保持名称/目录，视角差异不改成状态衍生。完整 prompt 可指定位置、朝向、尺度、遮挡与可见部分，不默认复制参考构图或自动建模。重生逐项经 asset-to-image-path 映射为有序 job.images，资产图在本地 PNG 后缀前，依赖无环；缺参考报告 prerequisite blocked，不清空或扩范围重生。修订绑定须在提交前独立 prompt 复审，参考变化只评估实际受影响证据。

按授权最小修正目标卡：表达问题调整 `## 图像生成提示`；形象事实需要改变时同步视觉/状态描述。保留身份、出场记录及无关字段。涉及剧本事实、资产清单或其他目标时返回跨负责人建议，不扩大编辑范围。也可仅提交诊断 findings。

修复成果是满足当前身份/外观、风格、画质、材质及剧情需要的可用资产图，或有依据的诊断及未解决项。按声明媒体职责定位根因，不为无影响细节重生，也不要求静帧复刻参考视频可承担的透视、遮挡、相机、布局或整体运动。基础图门禁不依赖未来视频，不声称未看视频已解决问题；实际跨媒体冲突交最终 shot-input 审核。解剖/拓扑标签本身不是重生理由，须有对本图用途、身份/质量、剧情关键特征/动作或用户明确要求的具体影响；非必要细节不可见不制造 unknown。意见仅属偏好或职责错置时交 Director 协调独立 Reviewer 澄清，不自行改判。当前独立 pass 即停止质量循环，可选精修仅在用户要求或新需求时进行；真实问题仍处理，证据失效先评估，失败重试默认无次数上限不变。
