...
- `${CLAUDE_PLUGIN_ROOT}/skills/reviewer-review-outline/series.md` — 系列的连续性、阶段推进与规划契约参考
- `${CLAUDE_PLUGIN_ROOT}/skills/reviewer-review-outline/short.md` — 短篇的注意力、情绪落点与结构参考

## 审核边界

本 skill 是可选规划审核；遵守共享规则的独立新 Reviewer context/主 AI relay 协议，只写 `reviews/{ep}/outline.md` 的 Markdown 意见与轮次，不增加 runtime kind/evidence，不调度修复或替用户批准。不以规划缺失阻塞无此请求的制作，不运行全生产 readiness。读取失败/无法判定为 unknown。
...
缺失、空白、读取失败或边界不清/冲突为 unknown，交顶层主 AI/Director 澄清，实际答复回原审核任务，不回退模板或其他配置、不替用户确认。已有明确决定复用。实跑并保留 sum、边界、退出状态；超界写具体意见，脚本无法执行为 unknown。不以修订次数或审美判断放宽预算，不要求补建未采用的大纲。

### 叙事判断

阻塞意见聚焦框架与确认意图；有价值的措辞或节奏建议可另列理由，不作为审美门禁。意见供生产 Director 协调，不要求作者逐条照搬或调用指定修复方法。

**质性检查项（四维）**（与 reviewer-review-novel 三维统一术语，本 review 额外多 1 维 arc 覆盖）:
...
写入 `reviews/{ep}/outline.md`（append 模式，同一目标重审串行）。

**Round 自检**：
1. Read `reviews/{ep}/outline.md`（若不存在，本次为第 1 轮；否则按最大标题轮号递增）
...
### 文件操作
- 使用 Write 或 Edit 维护 `reviews/{ep}/outline.md`（append 模式，详见审核记录）
