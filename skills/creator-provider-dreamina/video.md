...
Use the converter's final ordered resolved `references`, with independent 1-based counters for images and videos. Header identity images remain first in first-use order, followed by local media in manifest order. Local PNGs continue the image counter; MP4s use the video counter. Thus `[identity PNG, local MP4, local PNG]` binds `{图片1}`, `{视频1}`, `{图片2}` respectively, not a shared media index or local-only numbering. Each binding carries its actual identity or declared control/use; filenames and vague attachment descriptions do not replace tokens.

The current `storyboard-to-prompt` / `shot-inputs.mjs` converter already emits these Dreamina tokens. Treat that output as the current Dreamina execution contract, not evidence of provider-neutral syntax support. Authors retain source links and manifest uses; review the resolved prompt and ordered bindings before preparation. Preserve the converter's prompt/references through review and submission, without manual slot guesses, renumbering or submission-time additions. Corrections belong in owner-controlled sources and current review, preserving converter equality and runner/wrapper passthrough.
