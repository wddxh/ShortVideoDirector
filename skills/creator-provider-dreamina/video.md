...
### Whole-Frame Coarse Mapping

Before final authorship, follow the [existing-asset selection loop](../_meta/rules/visual-prompt-craft-common.md#已有资产选用闭环): within authorized stable sources and actual reference observations, align objects with established identity/form/material, relevant existing cards/images, each using shot's header and current slots. Reuse suitable established images; missing selection affecting identity/form/material needs correction, not treatment as imageless. Source-based description remains valid when no applicable image exists. Do not scan/upload the whole library, require every background object to have a card, or invent assets for unsupported extra objects; repair such media.

For missing headers, Creator supplies exact objects/shots/card-image paths and source basis/impact through Director to Storyboarder under precise write scope and stable dependencies. Other story/list/design changes go to their owners. After stable handback, rerun storyboard-materials-dreamina.mjs and read its actual ordered references and referenceSlots. Added header images can shift later identity and local PNG image numbers; never guess the next image token. Preserve header first-use asset union followed by manifest local media, without a parallel upload list. Semantically revise and check the whole prompt and affected ref.use texts against new actual bindings, including opening mappings and all timed passages; appending a token or submission-time mechanical replacement is insufficient. Self-check final --json and obtain applicable scoped independent evidence with current fingerprints; changed bindings do not waive gates.

Use actual reference inspection evidence to author the final prompt's opening mapping for every distinguishable coarse object/class, including floors, walls, background, environment components, furniture and items with no asset image. Write actual color/shape/location -> final object -> adopted spatial envelope (position, screen occupancy/scale, distance, orientation, route) -> established final shape/structure, softness/rigidity, material surface/light response and source-action changes. Repeated classes may be grouped without losing visual categories; no per-mesh quota or unrelated unseen objects. Bind uploaded images with actual material-tool tokens. For imageless objects describe established source scene/art direction; missing necessary design returns to owners, without invented assets/tokens or compulsory new cards/images.

Preserve camera/light/reveal/cut controls. A coarse envelope is not final box silhouette, proportions, topology, rigidity or material; dedicated shape/topology and fine refs retain declared structure/material purposes. Author each `ref.use` for its actual role and synchronize it with the complete task prompt. Apply [whole-frame mapping](../_meta/rules/visual-prompt-craft-common.md#全画面粗代理映射).

Global stable mappings provide a reusable basis, not a once-only rule. Timed passages may restate or fully introduce a main object's rough-proxy-to-final identity/shape/material mapping for attention, coarse-appearance confusion or concrete expression needs. Repetition is not limited to new objects/changes; no per-segment ban or count limit. Clarify new objects, scene ambiguity, identity changes, shape/material states or changed purpose. Avoid mechanically copying irrelevant whole-object lists; preserve purposeful emphasis and full source actions/state changes. Every explicit video use still gives source interval, output task interval and adopted dimensions, with object introductions as needed. Official advice offers material correspondence, story progression and useful restatement; whole-object coverage and locally concrete intervals are SVD refinements, not official mandates.
...
Apply [proxy authority](../_meta/rules/visual-prompt-craft-common.md#粗模控制与外观依据分离). Every final prompt follows official guidance for complete source action, with or without limbs or local fine performance: necessary ownership, preparation/execution/recovery and contact changes, without invented actions or frame quotas. Advice 7 separately prefers limbless rough models and emphasizes limb/wing imitation risk. SVD rough production covers whole-object staging/paths, camera, space/reveals, light, transitions and clock without arms, palms, wings or fine mechanism animation; mechanism simplification is SVD's boundary. Simplify rough signals, never prose. Detailed/static/action refs retain purpose checks, not source-semantic exemptions or compulsory rigs/identity regeneration. Map colors/shapes; remove internal trajectories, coordinates, camera cones/debug labels, retaining film text.
...
Creator writes complete fine actions in final prompt: who, which anatomical hand/part, ownership, facing/grip/contact, initial/intermediate/end changes, ordering/overlap, result and source timing. Rough-video `use` and bindings may control operating-region layout and overall object paths, not fine contact or mechanism motion. Omitted hands/contact details are not floating failures or unknowns. Wrong declared positions/paths/camera, premature reveals or UI-result clocks require media repair; prose or relabeling cannot reverse them. Unknown requires a necessary gap within adopted controls; unresolved concrete conflicts are needs_revision. Preserve complete clean/caption MP4s, PLAN and saved episode specifications under the [local delivery contract](../creator-local-reference/SKILL.md#saved-episode-media-specification).
...
Another hypothetical excerpt: assume the inspected video contains only a blue cuboid bag proxy, a gray floor and a gray rear wall. Source art already defines a soft cloth bag, matte stone flooring and gray-white plaster, with a push-in during output 0–2 seconds. No identity image is supplied. These assumptions illustrate mapping, not new facts for other projects:

```text
{视频1}的0–2秒对应输出任务0–2秒，采用布局、空间尺度与推镜。左前景蓝长方体代表软布袋，采用其位置、屏占尺度及朝向；袋身为源定柔软织物，圆弧袋底，自然褶皱与柔和漫反射。下方灰平面代表磨砂石材地面，后方灰立面代表灰白抹灰墙，采用二者的范围、距离与空间邻接；石面细颗粒、墙面细微抹痕均按已定美术呈现柔和漫反射。粗占位的包络用于空间关系，成片呈现上述真实形材。
[输出任务0s-2s] 参考{视频1}的0–2秒，仅采用低机位推近、袋与地墙的位置距离和屏占尺度变化；镜头逐渐靠近静置的软布袋，表面细节随景别显露。
```

Use those appearances only when real sources support them. Later passages may reuse or purposefully restate the mapping, including the bag's full final identity/shape/material if it is the focal object; source-required lifting, setting down or deformation is written completely at its actual task time. This excerpt supplies no asset/token invention, genre default or reusable COMMON draft.
