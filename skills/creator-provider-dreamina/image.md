...
Creator uses optional editable asset previsuals when exact geometry/topology, UI layout or multiview consistency need control beyond prose or existing references; identity roots without required refs may use text2image. Follow [local reference selection](../creator-local-reference/SKILL.md#choose-the-medium), keeping needed Blender/2D sources in the story project's `references/`. Provider wrappers and evidence checks govern paid output, recovery and acceptance; local craft does not bypass those safeguards.
...
Before prompt review, Creator resolves the final ordered `job.images` and writes the corresponding bindings into the card's `## 图像生成提示`. Each actual image has its token and concrete relationship/use: appearance/structure of the same entity or an included, held or worn asset, selected shared features, base state, or local control/placeholder intent. Number across the entire array: declared direct asset references and required base images first, then local PNGs in their existing declaration order. For `[identity.png, local-shape.png]`, `{图片1}` supplies identity and `{图片2}` supplies shape control; the first local PNG is not automatically image 1. With no references, use no reference tokens.
...
For optional `## 本地制作参考`, follow [the shared contract](../_meta/rules/local-reference.md). Run `node "${CLAUDE_PLUGIN_ROOT}/scripts/local-reference.mjs" parse|ready CARD` with one action. Basic refs keep required asset images first, then every declared local PNG once in declaration order as the exact suffix. The reviewed basic prompt must bind those actual inputs and include the narrative's control/placeholder intent; the wrapper does not append it. Sources are actual editable projects/scripts/inputs, not uploaded images. The runner/wrapper validate local readiness/order; direct asset-reference mapping remains Creator's responsibility.

Prompt review covers only authorized new/regenerate targets, reading declared local PNGs/sources rather than future output PNGs. Reused inventory is reference input, not a prompt-review target. Missing local inputs block submission. MP4 belongs in the [generation-task manifest](../_meta/rules/shot-inputs.md), with at least one full-group MP4 per task, not image refs or completed-video records. Sources do not upload. Image acceptance does not prove motion or replace shot-input/internal-cut/boundary review.

For a standard card's optional basic-info `资产参考`, Creator maps every declared canonical direct card link through `asset-to-image-path.sh`, in declaration order, into `job.images`. Each declaration includes prose relationship/use; bind these actual inputs globally and inline in the self-contained target prompt before review. Asset images supply appearance/structure; prose may set placement, orientation, scale, occlusion and visible parts. A window view, held/worn asset or shared-feature reference does not automatically require local modeling; use local PNGs only when complexity/precision needs more control. Shared features do not imply shared identity; actual same-entity views retain their common identity, topology, materials and state under the correct view.

Required generation dependencies must be acyclic. Do not recursively upload refs of refs, silently empty a missing reference, or add generation/force targets to obtain it. Authorized in-batch prerequisites wait; missing/unready external prerequisites block. Whole/part/views retain their names/directories, not state derivatives. Existing derivative base-image rules remain intact.

This is an LLM mapping contract, not a production declaration parser. The runner enforces dependencies and forwarding of actual images, not asset-reference semantics or declaration-to-images completeness. Receipts lack a reference list: their settings/status/output hash do not prove which original reference images were submitted.
...
One fresh Creator generation context handles one coherent, authorized, current-prompt-approved, ready multi-job batch through one runner as a bounded operation, following [visual context rules](../_meta/rules/visual-context.md). Default concurrency is 5, not an account quota or cross-process semaphore; use one runner, not simultaneous runners. Serial execution/concurrency 1 or other overrides need an actual scope, dependency, provider, local-resource or user constraint, briefly noted in the existing handoff, without a new report/schema or mandatory quota. Independent jobs overlap without waiting for another target's visual acceptance; shared review-file writes remain serial. Actual reference dependencies and evidence gates still wait for ready prerequisites, including fresh forced outputs.
...
Report the whole commissioned group after draining, not just the last worker or an early failing prefix. The generation context returns text status, all IDs and paths without reading or attaching images. Each visual inspection uses a fresh context and minimal helper thumbnails; acceptance belongs to fresh independent Reviewer tasks. Keep unchanged accepted evidence and all outstanding review targets; neither an empty success set nor a skipped PNG clears them. Actual updates may invalidate affected evidence and need independent review.
