...
3. 根据退出码处理：
   - exit 0，stdout 以 `SUBMITTED` 开头 → 提取 `submit_id`，记录 `{"shot": N, "submit_id": "xxx", "status": "submitted", "prompt": "替换后的完整prompt", "images": "图片路径列表", "duration": 时长, "fail_reason": "", "retriable": false}`
   - exit 1，stdout 以 `FAIL` 开头 → 记录 `{"shot": N, "submit_id": "", "status": "failed", "prompt": "替换后的完整prompt", "images": "图片路径列表", "duration": 时长, "fail_reason": "...", "retriable": false}`
