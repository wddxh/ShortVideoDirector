...
1. 先通读 outline，标出本集每个「主要事件」对应的画面场景、参与角色、关键信息点；**记下每场景 `目标时长` 字段**，按 rules.md「场景级内容密度预算」段算每场景字数区间（场景秒数 × 8-12 字/秒）作为写作时的密度锚点
...
- **字数与时长脱节** — 短场景写了大段独白（10s 场景塞 500 字 → scriptwriter 必须删）/ 长场景几句对白带过（30s 场景仅 50 字 → scriptwriter 必须补构）— 写完每场景默数字数对照 rules.md 密度档位
