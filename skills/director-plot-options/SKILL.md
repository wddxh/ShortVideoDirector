...
本 skill 是顶层生产 Director 的本地知识。intake 充分且获准创作候选后，主 AI 自己落盘到 `story/planning/plot-options.md`；仅诊断/待澄清时不创建。落盘不免除完整呈现义务，直接读取并展示当前完整候选与解释，不向自己转交文件或任务。

遵循 [项目布局](../_meta/rules/project-layout.md)：更新同一当前候选文件，修改前按需在 `story/decisions/` 保留完整题目、候选版本与原始答复/条件的关联。已有委托指定来源继续复用，不自行迁移；已采纳故事及关联修订与未选候选分清，不自动合并成新稿。
...
主 AI 作为 Director 直接与用户交互，编写全部可预见相关问题并标明题界及条件分支。“选哪个剧情”仍是一题，包含稳定标签、全部三个候选正文（明确数量则按该数量）、完整解释与取舍，以及“Director 决定”的范围。动机、冲突、推进和结局全部展开，不只给标题/类型/摘要；其他设置单列题界。完整计划内部保留，沿题界展示当前题全部内容并原生单选，不摘要或倾倒全表。自编计划的原始答复及条件本地保留；专家提供的计划完整展示，相关原始答复/条件批量回原专家任务，提前回询仅限共享规则例外。

仅需用户选择或到指定检查点时准备该包；主 AI 读全并展示所有当前候选与解释，再用原生单选控件，questions 恰好一项，三个候选加委托（宿主上限按共享映射保留全部选择并披露限制）。Markdown 不替代可用控件。等答复再问计划内下一适用题，保留原始答复及全部条件，不做自我 relay。已有选择委托时 Director 在范围内决定并记录依据/理由，不冒称用户亲选、不再求批准。协作按实际材料和授权，不规定调用链。
