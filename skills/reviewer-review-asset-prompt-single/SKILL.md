...
指定临时目录先存在；读取卡片/config 前以显式 SVD_CONFIG 执行 `review-round.mjs start asset-prompt EP TARGET STATE [EXTRA_INPUT...]`。新语义参考首次读取前 `add-input STATE PATH...`。审核 `## 图像生成提示` 的模型可消费性、语言和引用；衍生资产还核对基础资产/类型路径。读取「资产参考」必要直接卡及相关图像提示（可跨类别），检查 canonical 有序直链、prose 关系/用途与无环生成依赖。判断外观/结构、绑定或观察面需要已有直接参考 PNG 时，先采集再实际看图。参考仅作 inputs，不扩 target 或递归历史；未来输出不强制先存在，当前判断必要证据缺失为 unknown，已制成本地参考仍须实际查看。
...
将最终 prompt 与执行应提供的 refs 作为可消费性边界：核对「资产参考」到有序 job.images 的实际映射、资产图在本地 PNG 后缀前，以及每份输入的全局和内联用途绑定。参考供外观/结构，文字可定位置、朝向、尺度、遮挡与可见部分；选择正确来源和观察面，不要求复制参考构图或自动建模。仅真实同实体核对共同身份/拓扑，仅借鉴共同特征则限定借用维度。衍生图保留基础引用及当前状态。源剧本/别卡不能由审核者在心中替模型补齐，“手持那把剑”缺可见特征或实际绑定须指出。未来目标图匹配留 visual review，必要直接卡/图按共享证据规则在读取前采集；helper 不机械验证资产参考映射完整性。
