...
这是可选规划审核，不要求所有 series 创建 arc。遵守共享 review-meta-rules 的独立新 Reviewer context/主 AI relay 规则；只写 `reviews/story/arc.md` 的 Markdown 意见与轮次，不增加 runtime kind/evidence，不修复、不调度生产、不替用户记录制作前批准。输入缺失或无法判断返回 unknown。
...
审核结果写入 `reviews/story/arc.md`（append 模式，同一目标重审串行）。

**Round 自检**：
1. Read `reviews/story/arc.md`（若不存在，本次为第 1 轮；否则按最大标题轮号递增）
...
### 文件操作
- 使用 Write 或 Edit 维护 `reviews/story/arc.md`（append 模式，详见上文「输出格式」段的 Round 自检流程）
