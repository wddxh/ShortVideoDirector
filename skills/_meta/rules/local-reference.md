...
静态卡片参考说明当前目标用途，不虚构播放时钟。视频参考按 [参考区间与采用维度](visual-prompt-craft-common.md#参考区间与采用维度) 在 use 与最终 prompt 写明实际素材区间、任务区间及采用维度。上传素材清除内部轨迹线、坐标、camera cone 与调试标签，保留有源依据的正式片中文字；采用带肢/翼的粗参考时，最终 prompt 写相关完整动作序列，不扩成本地精细 rig。
