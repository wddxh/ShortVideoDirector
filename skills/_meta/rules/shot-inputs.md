...
Coarse references require sufficiently detailed shot prose: who does what, relevant body/head/prop facing and pose, screen versus anatomical left/right, ownership, grip/contact and necessary initial, intermediate and final transitions. Select detail by the action, not mandatory all-fields or detail quotas. Reference `use` declares proxy controls and limits, not final style. The final prompt interprets actual limb/contact proxies as the intended actor's anatomy, pose, grip orientation and action; assets and the work baseline provide final appearance. Production notes alone do not reach the model.
...
Assets primarily provide identity/appearance, style, image quality and materials. Local MP4 supplies declared perspective, occlusion, camera, framing, scale, layout and timed whole-object transforms; asset stills need not duplicate these controls. Actors default to rigid BOX proxies. When holding, support or action would otherwise be unreadable or falsely floating, include minimal hand/forearm, relevant limb or contact proxies, especially with the body offscreen but also for necessary onscreen support. This is within authorized local-reference scope; full rigs, detailed fingers and facial animation are not the default. Missing fingers is not failure; missing necessary support conflicting with action is. Judge intentional levitation, transparent screens and concealment by intent. Environment/prop geometry and static asset shape PNGs retain their scope. Preserve plot-critical features/actions and explicit user requirements.
...
Creator self-checks the actual final `--json` output, then commissions a fresh independent shot-input Reviewer for source fidelity, completeness and integration. Submission forwards the reviewed prompt exactly, without rewriting or appending bindings.
