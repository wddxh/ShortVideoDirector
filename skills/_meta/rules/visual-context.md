...
shot-input 独立 owner 在 start 后读取最终 prompt、timeline、manifest 与必要源文本，先规划相干视觉窗口、关键切点、接触/阅读阶段及必要外部配对。实际查看全部交 fresh Reviewer，用小型最小必要 helper 缩略图集；owner 始终不加载图片、帧、contact sheet 或图像附件。单资产视觉叶子任务仍可在其新上下文中查看有限比较集并完成自己的记录，不普遍加一层总审。
