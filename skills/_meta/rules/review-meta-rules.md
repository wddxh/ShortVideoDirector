...
以下标准按当前媒体职责使用，不把后续 shot 的证明提前压给基础资产图。按 [参考用途与精细度](visual-prompt-craft-common.md#参考用途与精细度) 区分粗代理与细模实际 purpose：细模可采用已有结构/材质，也可只借空间、不抄动作，不套粗代理省略要求。粗白模普通移动优先无肢/翼 BODYBOX；必要支撑保留相容姿态随整体移动，不推导步态或摆臂。操作姿态限对应阶段及准备/收尾，核对进入/退出与有意连续性，不自动重置或跳隐。带肢/翼参考的相关完整动作序列须在最终 prompt 表达，不要求扩建本地 rig。受托特殊动作选择关节时序控制时，必要中间运动须有证据；碰撞检查仅辅助。

省略解剖细节、块状风格、静帧保持、刚体平移和固定代理姿态本身不失败；结合实际 use 与最终 prompt 判断声明的控制是否违背源事实。反向肢体、穿身或无支撑握持等只有实际误导归属、接触、空间、揭示或关键动作时序才阻塞，不从粗姿态推定最终人物僵硬。正常肩部连接和合理遮挡/投影重叠不要求零相交，不增完整解剖门禁。具体冲突优先省略非必要代理或调整合理姿态/机位/支撑，保住必要动作；未解为 needs_revision，必要控制证据不足为 unknown。清楚表达抽象与自然动作的分工可处理有意省略，不能反转矛盾媒体事实；悬浮、透明屏幕和遮挡按意图判断。
...
粗参考不能替代完整 shot prose：按动作核对谁做什么、必要朝向/姿态、左右、归属、握持/接触及初中末变化，不做细节配额。按 [参考区间与采用维度](visual-prompt-craft-common.md#参考区间与采用维度) 核对 ref.use 与最终 manifest.prompt 的实际素材到任务时窗、用途和代理映射；静态图不虚构播放时钟。最终 prose 写源支持的自然动作与意图，局部动作不扩成全片循环，不自动加手势。上传媒体清除内部轨迹线、坐标、camera cone 和调试标签，正式 film text 保留。职责清楚的抽象不因假想模仿而返工，具体源冲突仍须修，文字声明也不是成片保证。
