...
需要预览工具的 Reviewer 自行查询固定 `story/work/shared/environment/environment.md`，纯文本角色不强制读取。报告由 Creator 在项目初始化的有界授权内全面检查并撰写，工具范围与完整职责见 [环境报告规则](user-decision-relay.md#local-environment-and-method-choice)。固定报告是常规 read 依赖；Director 集中维护初始化/更新的单 owner writer 占用与 stable/等待条件，读者等实际写入完成，不假定永久稳定或要求逐任务转传结果/路径。

Reviewer 只读报告；实际失败、环境变化或确切新增需求所需能力未被初始化证据覆盖时，沿 handoff 交 Director 有界授权单一 Creator 补验更新受影响项，保持稳定性保护。新增能力只验证该需求的未覆盖部分，不自修、不每任务探测、常规扫描版本或无理由重复验证。报告中的工具指导默认不进 manifest.sources，也不默认成为每份 review 的语义 input。确以报告内容作验收判断依据时，目标 owner 在该语义读取前 start/add-input；视觉 helper 使用该依据前同样由 owner 采集。普通工具指导读取仍遵守读写占用，但不因此自动哈希。

环境可用性不指定制作工具，也不证明材料通过。审核按源意图、实际媒体和声明控制判断；静态设计与时间合成可分开选法，作者可复用相容素材或在授权内换方法完成返工。除真实用户要求或有据的当前局部选择，不把 SVG、批量 `.blend`、CUDA 或某种后端设为门禁；必要媒体、独立视觉证据和既有验收契约仍须满足。

## 当前范围证据

独立审核与重审是内部自动质量门禁，不自动产生用户批准步骤。生产 Director 在原授权内协调修复并重新送独立审核，不能以用户同意伪写独立 pass，也不逐轮请用户批准。用户明确要求当前任务视觉验收例外时，仅按下述正式通路记录独立的用户接受依据。新问题先依据当前配置、材料、grants 和专业判断处理；仅用户指定检查点、缺必要权限或无法内部解决的关键冲突才按 user-decision-relay 交回完整决策包，只暂停受影响工作。
...
shot-input 由纯文本独立 owner 总审：start 在读最终 prompt/timeline/manifest/源之前，读全后按 purpose/use 规划空间、整体轨迹、摄影、光照、揭示、切点/时钟及阅读/结果、外部配对窗口。所有最终 prompt 无论参考有无肢翼、是否演细节都核对完整源动作序列、主体/部位归属、必要准备/执行/收尾与接触变化，不编动作或设每帧配额。helper 另查粗视频及源中的肢翼/精细机构信号与具体矛盾，优先省去信号复用，不省文字；保留肢翼另处理模仿风险。独立 action reference 按所选动作查证，不倒逼创建。owner 不加载图片/帧/contact sheet/附件；查看交 fresh Reviewer 最小 helper 缩略图集。单资产叶子仍可有限查看写自身记录。
...
卡片本地参考沿用 asset evidence；最终生成任务 manifest 用 `shot-input` kind，见 [shot-inputs.md](shot-inputs.md)，保留五种 kind。输入含目标 manifest/config/script/storyboard、上传 PNG/MP4、sources 及实际资产视觉依赖，不给每项媒体审核附整个装组计划哈希。聚焦最终 prompt/media 集成、变化细节、任务时钟、内部切点/声音桥及必要外部边界，无具体冲突复用 storyboard 判断。同组源 `视频风格` 精确相同、materials 提取一次；最终 prompt 表达统一基线，可结尾重申必要全局要求。动作表情、意图与声音保留 prose，use 声明实际控制权限和区间。按故事选相邻/非相邻/跨集配对，比较位置、轨迹、状态、轴线与身份，实际依赖入 inputs 指纹并说明覆盖限制。必要运动不可核实为 unknown；变更先评估影响，不自动重渲染。

基础卡「资产参考」是 Reviewer/Creator 的语义与映射责任。`asset-prompt` 读取必要直接参考卡及其中相关图像提示；判断绑定、外观/结构或观察面需用已有参考 PNG 时也实际查看。未来目标输出及已授权、尚待批内生成的普通参考 PNG 不要求预先存在；若当前判断确需图片而缺失则说明缺口、记 unknown。`asset-visual` 实际读取必要直接卡/提示与对应参考 PNG。所有实际读取均在首次读取前由 start/add-input 采集；本地声明 PNG/源码仍按既有契约必读。

单项始终一个 TARGET，直接参考仅作 inputs，可跨类别，不递归参考链或历史、不扩 scope。核对声明关系/用途、实际来源与正确观察面：包含另一资产、持有/穿戴及共同特征按各自用途比较，不照抄参考构图；仅真实同实体才要求相应共同身份、标志物、几何、材质与状态一致。借鉴共同特征不强制同身份，无制作影响的细像素差异不作门禁。当前 helper 不解析资产参考声明或强制其输入完整性；它检测已记录参考哈希变化，但漏记仍可能机械通过，不能用 helper pass 代替此核对。
...
shot-input 纯文本 owner 按 [接点审核](../../reviewer-review-shot-inputs/SKILL.md#task-junction-coverage) 判断每个相关任务接点：整集覆盖所有相邻组（含场/幕变化），局部只取必要邻界及真实非相邻/跨集依赖，不附全计划哈希。fresh helper 将实际选中 clean MP4 的尾/头相干窗口作为一个有限比较，结合两端最终 prompt，按源意图区分同一事件的必要动作/空间/声音延续与场幕/时空跳转的叙事关系及观众定位，不能仅匹配端点或把续动作/续声强加给转场。披露实际听见、仅有音轨、计划声桥和假音频的区别；相同音色描述不证明独立生成音频可无缝接续。

每个消费 owner 在 helper 首读前已采集对应 inputs、版本与问题/窗口仍适用时，可复用独立配对观察，各自判断本目标，无需两端重复视觉 pass、互相依赖 review 文件或新账本。后发现消费方及变更按既有采集/兼容性规则处理，不能追认阅读。缺必要邻组证据只使受影响 target unknown，邻组只读参考不扩生成授权。验收对象仍是参考与最终 prompt，不是生成视频质量或自动剪辑权限。

## 4 条核心规则

### 显式单任务人工视觉例外

仅用户明确接受指定任务当前视觉输入时适用，完整契约与命令见 [人工视觉例外](shot-input-visual-exception.md)。独立 Reviewer 仍写 shot-input/unknown，只有非视觉集成及必要外部边界已通过、剩余全部 blockers 仅为本组视觉证据缺口，才可提供结构化资格。Director 不能补造资格或将用户 PASS 转成独立实看。用户决定不构成生成/重试 grant。

`check-target` / `checkCoverage` 保持真实独立状态；就绪 `check` 可在其他门禁全部满足时以 `user_visual_exception` 接受该项，并同时输出 `shot-input-review:unknown` 与明确的 acceptance 行。完整凭据绑定项目/config/目标/全部成员、用户来源、资格轮次及当前必要 inputs，任何绑定变化均失效；无自动续期、覆盖或回退旧轮。
...
提示表达按 [任务提示组织](visual-prompt-craft-video.md#任务提示组织) 与 [每任务独立语义写作](visual-prompt-craft-common.md#每任务独立语义写作) 核对源忠实度、完整性、当前适用性和实际参考用途。官方建议 1–4/7 及建议 5 转场在其范围内优先，建议 6/extend 排除。公式可指导组织素材用途/时窗、概述、时间动作与情绪意图及可选全局重申；不把段标题、词数、句数、面部 3–4 个细节或相同用词变成验收配额。逐 task 亲写、use 同步及独立稿批量序列化均按共享契约判断，不能仅凭相似措辞推定模板代写。

通读实际 prompt 与 uses，定位不存在的事件/角色/声音、不适用分支、区间或控制权限串用及矛盾，给出源依据和制作影响。统一基线与必要共同事实可重申，局部规则不因放到结尾而变成全片条件。按 [转场过程表达](transition-craft.md#转场过程表达) 核对两端、触发、过程与声画窗口，区分淡黑后显现的先后和交叉叠化的重叠；有效硬切不需中间动画，示例秒数不作预算。
...
以下标准按当前媒体职责使用，不把后续 shot 证明提前压给资产图。按 [参考用途与精细度](visual-prompt-craft-common.md#参考用途与精细度)，细模结构/材质或仅借空间、静态 shape/identity、精细几何与独立 action reference 按 purpose 审核；用途不同不免最终源语义完整，不强制降 BOX 或重生身份图。SVD 粗模只制作整体站位/轨迹、摄影/运镜、空间、光照、转场和时钟，不制作手臂、手掌、翅膀或精细机构动作；机构简化属 SVD 边界。无论参考有无肢翼、是否演细节，始终按官方表达要求核对完整源动作、部位归属及必要准备/执行/收尾/接触变化。另查已有粗动画信号，优先省去肢翼后复用；保留肢翼按建议 7 处理额外模仿风险，不是完整性触发条件。独立动作参考另查所选证据，不倒逼细 rig；碰撞检查仅辅助。

省略解剖/接触、块状风格、无肢粗模静帧与刚体平移不失败，也不产生本地动作证据缺口。无手占位按操作区域/整体关系判断，不自动等于错误悬浮；无肢只省本地表现，不省最终源动作序列。所有 prompt 均核对完整持有/接触归属及变化，带肢翼另处理模仿风险。错误构图、路径/机位、提前揭示、切点、UI 时钟或具体源矛盾先简化或修媒体，不靠“忽略”免责，不穷举潜在行为。正常连接/遮挡不套零相交门禁。明确冲突或任何必要动作描述缺失为 needs_revision；unknown 指出必要证据缺口，不因未演细节要求补手/关节动画。

具体动作、姿势和表情由完整 shot prose、最终 prompt 与模型负责；核对初中末、先后/重叠、归属、触发反应与结果，景别/角度支持最终动作可读。粗视频提供操作区域、整体关系与相机/时钟，源 clock 上哪些事实须由影像呈现取决于媒体职责，不能全推给文字。环境/道具可保留镜头/布局所需几何，静态形状按声明审核；详细外观遵循统一作品基线和实际资产。缺已声明媒体、必要输入不可读、指纹漂移或声明的必要轨迹无法评估仍为 unknown，独立审核与既有门禁不变。
...
粗参考不能替代完整 shot prose：按动作核对谁做什么、必要朝向/姿态、左右、归属、握持/接触及初中末变化，不做细节配额。按 [参考区间与采用维度](visual-prompt-craft-common.md#参考区间与采用维度) 核对 ref.use 与最终 manifest.prompt 的实际素材到任务时窗、用途和代理映射；最终正文每次显式视频引用就地写具体源视频区间、采用维度和输出任务时间，全局同钟声明不能替代局部绑定，静态图不虚构播放时钟。最终 prose 写源支持的自然动作与意图，局部动作不扩成全片循环，不自动加手势。上传媒体清除内部轨迹线、坐标、camera cone 和调试标签，正式 film text 保留。职责清楚的抽象不因假想模仿而返工，具体源冲突仍须修，文字声明也不是成片保证。

TASK 是生成单元，不等于场景或幕，约束内可含多镜/多场，不强制幕结构。每个接点按 [源意图](shot-inputs.md#reference-authority) 判断：同一连续事件核对必要动作进度、持有/接触、空间与声音的相容延续，不要求相同帧或静止接静止；场/幕或时空跳转判断因果、情绪、信息、主题反差或平行关系与观众定位，不强制同位置、续动作、连续声音或过桥场。源支持的悬念、突兀感与硬切不必顺滑或立即解释；同集底层身份与世界事实一致，有意变化须有源依据。无源依据的停步、终姿、停顿或重启若破坏节奏，应定位说明。

独立 TASK 边界按 [装组契约](shot-inputs.md#manifest) 默认强烈优先有动机、明显不同的机位／视点／景别，以降低近似独立生成差异的显眼程度。实际需要的匹配构图及关键接触／必须无缝续声同组可有据保留，取舍不新增许可；不设每镜变化或角度配额，不保证连续性或豁免违背源意图的身份／状态／动作错误，重复构图本身不失败。保留模型最大值、源时长、连续成员和 grants；源重设计交 Director/owner 在原始预算内同步，不静默重组或强制叠化。

修正意见连接实际观察、制作影响与目标状态。材质意见可说明反射或纹理尺度如何影响已确认设计，再给出适用的表面表达；表演意见可说明观众需要感知的转折，再建议动作、声音或剧本已选择的内心独白。示例用于解释方向，最终表达由对应作者完成。

例如：“两端 shot prompt 的道具持有手不一致，会造成动作跳变；建议核实终态并明确当前起始关系。”观察、影响和方向可验证，不替作者定稿。BOX MP4 核对位置、整体轨迹、轴线、操作区域、揭示与时钟；所有最终 prompt 均依源核对完整持握/接触序列，无论参考有无肢翼或是否演细节。粗动画肢翼优先省去信号再用，保留则另查模仿风险/具体矛盾；独立 action reference 另审所选动作。构图偏好另列建议，不放 blockers。
