...
3. 主动读相关文件锁定用户提及的具体位置；若用户输入未明确位置，先澄清
...
> 此请求超出当前 edit skill 范围：{具体原因}。请手动修改 {建议位置}。
...
8. 某节点 skill 调用失败（非 review 失败）→ 该节点终止并中断后续级联，在阶段 4 摘要中报错
...
| Edit asset-list 清单 | 直接用 Edit 改 `story/episodes/ep01/outline.md` 的「本集资产清单」部分 |
