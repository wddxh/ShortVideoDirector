...
Use [camera-language knowledge](../storyboarder-storyboard/camera-language.md) for staging/coverage, viewpoint, lens-distance-focus and continuity. Compare script beat meaning, current reviewed shots and final refs: has framing/occlusion hidden necessary evidence, focus or movement redirected attention, or a cut/overlap changed when a reveal or reaction can be understood? For example, all actions can remain in the prompt while a close-up removes the listener's only visible response. Reopen established camera decisions only for such actual integration changes, not preferred coverage. Local BOX previews need appropriate camera/layout/trajectory, not rendered hands, rigs or facial performance.
