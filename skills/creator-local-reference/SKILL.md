...
Use cheap rehearsal inside local-reference when a concrete question needs time as well as space: can the reveal be read before the cut, does the listener's turn compete with a key line, or can the camera find the prop without losing the reaction? Start with rough BOX camera/staging and the relevant [camera-language knowledge](../storyboarder-storyboard/camera-language.md), not polished assets. Increase fidelity only to answer the unresolved question, such as readable prop scale, occlusion or focus depth; keep the same no-hands/no-rigs scope.
