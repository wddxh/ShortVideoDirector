...
At the Director-coordinated production start, establish the native OS/architecture, executable resolution and versions for selected drawing/compositing tools, `ffmpeg` and `ffprobe` using native lookup (for example `command -v` on POSIX). Check only planned dependencies: Python/Pillow for previews, SVG rasterizer/fonts when SVG is used, or Blender when needed. Reusing suitable images/clips does not require an SVG setup. Do not assume another machine's paths, versions or devices apply. Missing optional tools limit dependent work, not all craft; config-only calls do not initialize or render.

Run a tiny representative drawing/scene through the selected renderer in its intended headless/display environment; keep probe code/logs/outputs in scoped `/tmp/opencode`. Confirm actual output and runtime backend/device evidence. For Blender, executable presence, device enumeration or `nvidia-smi` is insufficient: CUDA compute does not prove Workbench/Eevee graphics or vice versa. Exercise only needed paths. Verify planned FFmpeg encoders/filters with a tiny output and `ffprobe`, not listings alone.
...
For planned local animation/composition, include a tiny MP4 in the selected tool-path probe, not only a still. Confirm decoding, codec, dimensions, rate and duration. For Blender prefer an appropriate GPU engine proven under the actual runtime environment; explain unavailable/unsuitable GPU paths and tested CPU fallback. This proves tool operation, not production framing or motion. Do not hardcode a GPU model, OS or backend, or launch a full animation to discover whether the path works.
...
The optional renderer needs Python 3, system librsvg >= 2.46 with Cairo/GLib/GObject via stdlib `ctypes`, and FFmpeg with `libx264`; it does not use CairoSVG or require Pillow. Check these only when selecting this tool; no auto-install or new readiness gate. Frames pipe to one FFmpeg process without per-frame SVG/PNG files. Output is silent H.264/yuv420p; retain required audio separately during mixed assembly.
