...
Titles, rough sound or timing guides are optional communication aids within scope, not automatic final editing. Each [task manifest](../_meta/rules/shot-inputs.md) selects at least one full-group MP4 as `kind:local,media:video`, with actual sources and use. Derive its reference clock from unchanged member durations; align internal cuts, camera/BOX trajectories and intended sound bridges to task intervals. Static intervals may use static clips. PNG supplements static controls; GIF is unsupported. Reference media is not submission authority or task completion. Inspect meaningful internal cuts and external boundaries with disclosed viewing limits, not endpoints or ffprobe alone.
