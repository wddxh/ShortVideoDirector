...
```bash
python3 scripts/svg-animatic.py SOURCE.py --duration 6 --fps 24 --width 640 --height 360 --output ABS.mp4 [--overwrite]
```

In production, resolve the tool under `${CLAUDE_PLUGIN_ROOT}/scripts/`; place trusted author SOURCE and actual editable dependencies under the story project's `references/`, not the plugin/examples. Read SOURCE before executing it: it runs once with caller privileges, not in a sandbox. It defines `frame(t, width, height) -> str`, returning a nonempty static SVG document. Each call gets exact `Fraction` time `n / fps`; compute motion/state in Python, not SMIL, JavaScript or CSS animation. The caller's working directory is retained; use source-relative paths deliberately and self-contained SVG (embed images; no relative SVG base URI).
...
The helper verifies the CFR grid using `ffprobe` frame timestamps/durations and stream duration, allowing at most one source timebase tick of quantization, not cumulative drift. It preserves the source's rational frame rate (including `30000/1001`) and frame count. Verify the caption output matches clean duration, frame rate and cut positions; do not retime the source for subtitles. VFR/unverifiable timing is rejected, and frame inspection times out after 120 seconds. Report such limitations to Director/main for coordination; a shortened preview cannot fulfill complete-task delivery. The helper does not automatically normalize or change source rate.
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/episode-previs.mjs" EP --parts PARTS.json --output OUTPUT.mp4 [--font FONT]
```
...
The interface supports same-FPS CFR, square-pixel clean inputs whose real video durations match canonical group durations. Verify actual streams/frame timing; mixed dimensions are padded to a common canvas without scaling or cropping. Different FPS or other incompatible media require Creator's scoped compatibility repair before assembly; report the limitation rather than claiming completion or silently retiming sources. Preserve audio where present; when only some parts have audio, fill the others with equal-duration silence. When all parts lack audio, keep the output without an audio track. No TTS or seamless independently generated J/L audio is promised.
...
Successful stdout JSON contains `output`, `ep`, `tasks`, `shots`, `duration`, `fps`, `audio`, `dimensions` and `mapping`. `output` is absolute; tasks/shots are in source order, duration is the canonical seconds total, fps is a rational string and audio is a boolean. Dimensions contains `width`, `picture_height`, `band_height`, `height`. Each mapping entry includes part/task identity, shots, absolute video/plan paths, canonical start/end/duration and episode-global shot timeline, plus input dimensions, padding left/top and audio presence. Treat this as the actual invocation result, not a persistent ledger or a visual pass.

Inputs must have the same verified rational CFR, with an integral canonical-duration frame count that matches exactly. The shared preview checks also require even dimensions, width >= 64, square pixels and no nonzero rotation. The canvas uses maximum input width/height with even-offset padding; the caption band is added below it. Present first audio streams are aligned to video start and converted to 48 kHz stereo/AAC, padded/trimmed to canonical part duration; missing audio gets matching silence only when another part has audio. The tool checks clean and caption episode clocks before publishing the new file. A nonzero exit reports `episode-previs:` diagnostics; temporary concat/PLAN/caption files are not formal deliverables.
