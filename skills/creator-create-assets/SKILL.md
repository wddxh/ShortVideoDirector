...
- 资产参考：按 rules.md 在基本信息用可选「资产参考」有序直链声明实际需要的卡及 prose 关系/用途，可跨类别用于同实体视图、包含另一资产的窗景、持有/穿戴或共同视觉特征。图像提供外观与结构，prompt 写明位置、朝向、尺度、遮挡和可见部分；简单关系不自动增加本地建模。必需生成依赖无环，不递归上传关联卡。确属同实体时先判断现图复用，再设计必要独立视图，保持名称/目录及共同标志物、几何、材质与状态；仅共享特征不强制同身份，视角差异不是状态衍生。
- 视觉细节：人物按 [rules.md 的七维表达](rules.md#人物提示的七维表达) 组织年龄/种族、肤色/皮肤质感、面部细节、眼神/表达的底层情绪、发型/发色/环境互动、服装剪裁颜色材质穿着方式、体型情绪气质。按目标选择适用细节，面部 3–4 项组合是建议而非每卡配额；真人美型可保留真实皮肤质感，动画按风格取用，机器人/非人用物种结构、传感器、外壳等相容表达。物品描述材质纹理，场景描述光线与空间。
- 美术交接：向 Storyboarder 提供作品共同造型、材质和渲染基线，由其写入各源 shot 的 `视频风格`。每个资产图与视频请求独立表达相容的适用基线，允许在结尾等位置有目的地重申必要的全局约束，不逐镜机械重复；reference use 仅管控制用途。
- 模型与语言：提示格式适配当前目标模型，语言遵循实际配置。
- 本地表达：精确形体/拓扑、UI 布局或一致多视角需要文字/已有参考不足以提供的空间依据时，选用 [creator-local-reference](../creator-local-reference/SKILL.md) 制作可编辑静帧到故事项目 references/。资产预视按需采用，不必每卡建模；无必需引用且可由提示建立外观的身份根资产允许 text2image。按共享 local-reference.md 添加可选声明，保留实际源码/工程/输入；基础提示绑定真实参考并写全 narrative 的控制与占位边界。纯补卡委托不自动扩为本地制作；已授权制作内的必要参考不另问许可。
- 身份基准：中性表情、自然站姿和清楚面部有助复用，但不强制日常便装。制服、礼服或护甲可作为代表性基础形象；显著且剧情需要的特殊状态另建衍生资产（适用四类）。
- 衍生边界：对比基础资产，无明显视觉差异不另建；基础资产字段须指向非衍生资产，禁止递归派生。

### 设计核对

- 用轮廓、适用的面部或物种结构、材质与色彩保留身份，完整描述设定服装；统一艺术风格保留角色差异，也保留用户明确设计的同源脸，不强制改成不同面容。
- 同一次实际提交的故事 prompt 已有 camera/action 时，人物小节可省重复机位和动作；必要身份与服饰保持完整。独立资产图请求仍写全自身目标，不依赖未提供的故事文字。
