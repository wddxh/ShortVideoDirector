...
本 skill 是顶层生产 Director 的本地知识，主 AI 直接组织确认说明并与用户交互。自编计划和实际答复本地保留；专家决策包按共享规则完整呈现并将相关原始答复/条件批量回原专家任务，不做主 AI 自我 relay。
...
按 [项目布局](../_meta/rules/project-layout.md) 选择必要文件：完整问题计划与原始答复/条件需要持久保存时放在 `story/decisions/`，关联实际材料；简单澄清可只回文本，不为确认步骤新增状态账本。
...
- 读 `config.md`
- 先读用户提供的材料；按 mode 指引选择有关的既有规划、剧本或小说。小说、章节和节选沿用实际路径，确认采用范围与保留要求后交 Scriptwriter 改编，源文保持原样
...
核对确认说明是否忠实、是否遗漏影响决策的约束，再直接呈现当前问题；自检不代表用户已批准。

## 输出

仅有真实待决事项时，编写全部可预见相关问题/表，标明题界、全部适用选项/解释、稳定标签、背景取舍及条件分支；费用不自动设字段。主 AI 读全后内部保留计划，仅沿题界完整展示当前题，再用可用原生键盘选择器，questions 恰好一项。自编计划答复本地保留；专家计划的原始答复及全部条件批量回原任务，不逐题往返。仅缺内容/映射、不相容或计划外决定才提前回询；不推断专家条件，不有损改写或倾倒全表。长解释在控件前，限制按共享映射披露。以下只是相关内容参考：
- mode 标识
- (series) 总集数 restate
...
- 指出可直接采用的剧本内容，不把确认说明变成重新创作。Director 直接处理制作前确认要求，不能以自检代替用户批准。
