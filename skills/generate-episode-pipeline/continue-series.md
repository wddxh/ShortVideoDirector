...
14. 使用 Skill tool 调用 `storyboarder-storyboard` skill，参数 `{ep}`。
15. 使用 Skill tool 调用 `director-review-storyboard` skill，参数 `{ep}`，按通用 review 循环处理。
16. 使用 Skill tool 调用 `creator-storyboard-sheet-prompts` skill，参数 `{ep} full`。
17. 使用 Skill tool 调用 `director-review-storyboard-sheet-prompts` skill，参数 `{ep}`。
18. 使用 Skill tool 调用 `creator-generate-images` skill，参数 `{ep} storyboard-sheets`。
19. 使用 Skill tool 调用 `director-review-storyboard-sheets-visual` skill，参数 `{ep}`。
