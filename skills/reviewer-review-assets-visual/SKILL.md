...
沿用委托/作者计划说明各目标的必要共享参考，不增账本。每个消费目标 owner 都须在共享参考首次读取或交 delegate 前用自己的 STATE 采集；已知有效项目相对路径前置到各自 start extras。delegate 返回实际源路径、观察和查看限制；`/tmp` 诊断只作反馈，不能复制为项目证据。仅证明 ready 的易变状态/审核文件不加入 inputs，实际依赖的语义审核仍须采集并协调稳定顺序。
...
每目标委托全新 Reviewer，提供 ep、asset_path/image_path、canonical review 路径、实际配置和最小必要参考。无读写/输入依赖冲突的就绪单项并行直写各自文件并返回路径/result。每次图片操作都新 task、缩略图优先，追加查看另派新任务。同一 ep/kind/target 的重审串行是输出所有权规则，其他读写/输入依赖仍须排序；缺参考、宿主资源或用户约束可支持有界分批/串行，在 handoff 简述原因，不设固定任务数或模型配额。缺参考目标仍保留在请求范围，不阻塞其他无依赖冲突的就绪目标。
...
计数以最终机器 result/finish 摘要为准；证据降级后原 commentary 可能保留旧判断，应结合最终状态提示阅读，不据旧 STATE 或原意见报 pass。真实漂移保留 unknown，后续按共享规则做独立 scoped 兼容性评估。
