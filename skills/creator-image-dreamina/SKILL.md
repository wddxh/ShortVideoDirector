...
使用即梦CLI为指定资产生成参考图片。按"基础资产 → 衍生资产 → 关键帧"三档拓扑顺序生成，每档全部完成（含 pending 轮询）才进入下一档：基础资产用 text2image，衍生资产用 image2image（1 张参考图 = 基础资产图，基础类型按 `## 基本信息` 中『基础类型』字段动态推导 character→characters / location→locations / building→buildings / item→items），关键帧用 image2image（1–10 张参考图 = composition 引用的所有资产图）。处理登录检查、逐档生成、pending 轮询和超时记录。
...
- **档 1 基础资产**（类型 ∉ {造型变体, 衍生资产, 关键帧}）— 第 1 档生成，无参考图，text2image
- **档 2 衍生资产（含旧造型变体）**（类型 ∈ {`造型变体`, `衍生资产`}）— 第 2 档生成，1 张参考图（基础资产图），image2image
- **档 3 关键帧**（类型 = `关键帧`）— 第 3 档生成，N 张参考图（composition 引用的所有资产图），image2image

按"档 1 → 档 2 → 档 3"严格串行：每档全部资产处理完（包括 pending 轮询全部结束）才能进入下一档。档 2 依赖档 1 的基础资产图，档 3 依赖档 1 + 档 2 的全部图。
...
3. 从 `## 基本信息` 提取基础资产路径：
   - 类型 = `造型变体`（旧）→ 读「基础角色」字段链接，推导 `assets/images/characters/{基础角色名}.png`
   - 类型 = `衍生资产`（新）→ 读「基础资产」+「基础类型」字段，推导 `assets/images/{基础类型}s/{基础名}.png`
     （基础类型 → 目录映射：character→characters, location→locations, building→buildings, item→items）
   确认该图片存在后作为参考图
4. 若基础资产图片不存在 → 记录失败（"基础资产图片缺失（衍生类型: {造型变体|衍生资产}），无法生成衍生图"），跳过该资产
5. 使用 Bash 执行：`bash ${CLAUDE_PLUGIN_ROOT}/scripts/image-gen-dreamina.sh "{提示词}" "{输出路径}" "{比例}" "{分辨率}" "{模型版本}" "{基础资产图片路径}"`（第 6 参数为单个 png 路径）
6. 按三态处理结果
...
- 档 2 衍生资产（含旧造型变体）：成功 N 张 / 失败 N 张 / 超时待查 N 张
...
- **跨档并行** — 档 1 还有 pending 就开始档 2 → 档 2 依赖的基础资产图可能还没下载完 → 档 2 大量"基础资产图片缺失" — 严格按档串行，每档 pending 全消化才进下一档
...
- **衍生资产基础类型字段缺失** — 衍生资产 .md 卡未写「基础类型」字段 → 无法推导基础 png 路径 → 档 2 直接跳过 — 必须按 `creator-create-assets/rules.md` 衍生资产模板补全 `基础资产` + `基础类型` 字段
