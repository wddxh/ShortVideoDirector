...
- **镜头时长合理性** — 逐个镜头检查内容总量是否与标注时长匹配。检查步骤：
  1. **非台词时间（估算）**：估算每个时间段内的角色动作执行时间（简单动作 1-2 秒，复杂动作 3-5 秒）、环境/氛围描写展示时间（静态场景 1-2 秒，动态场景 2-4 秒）、转场效果时间（通常 1-2 秒）
  2. **台词起止时间（推算）**：用时间段时长减去非台词时间，得出台词可用时间，推算台词的起止秒数
  3. **台词语速验证（精确计算）**：使用 Bash 调用 `bash scripts/speech-rate.sh "起始秒-结束秒:语速类型:台词文本" ...` 验证台词是否能在可用时间内自然说完
  4. **总时间判断**：若台词语速超标（OVER）或非台词时间已占满镜头时长导致台词无可用时间，说明内容过多，应建议拆分镜头或精简内容（优先精简台词和动作，保留视觉修饰细节）。内容过少应建议充实细节或合并镜头
