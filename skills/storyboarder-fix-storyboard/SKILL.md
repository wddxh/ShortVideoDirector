...
- 约束溯源：核对争议要求来自用户、必要叙事事实还是自选实现；摄影目标与任意窄参数分开。重复不可行时先查实际对象、有效面、可见性/可达性与已知路线几何，由 owner 在授权内改覆盖、站位或切点，不继续穷举微小参数或反复求许可。左右疑点先核对视角/镜面；图像依据不清先请求 fresh 独立检查，不据猜测替换。
- 修改范围：只修改意见涉及的 shot；增删或重编号时同步更新授权范围内受影响引用，范围外报告影响。
- 依赖检查：修改前后核对相关人物/道具的折叠状态、归属/持有、位置和行动后果，包含非相邻依赖；保留可信省略，不强制逐步展示或固定动作秒数。需扩展 shots 或写入路径时先通知原委托方，协调后再改；共享 storyboard 不因分章节就允许并发写入。
- 连续性与预算：按 [摄影知识](../storyboarder-storyboard/camera-language.md) 修订调度、覆盖和焦距/距离/焦点，保留节拍初态、必要证据、先后/重叠及注意关系。主动使用用户原始集目标已确认的 ±10% 创作预算，跨场景调整交 Director 协调 Scriptwriter 更新 canonical 目标，再同步分镜和受影响下游；范围内不逐次求许可。重算整集，精确要求优先，原始基准不滚动。
- 产物边界：仅修受托分镜，报告受影响资产、本地参考、manifest 与必要连续性依赖；Director 协调兼容性评估和独立 shot-input 重审，不自动递归重建。
