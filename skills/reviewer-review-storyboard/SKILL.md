...
按 [任务提示组织](../_meta/rules/visual-prompt-craft-video.md#任务提示组织) 理解官方建议 1–4/7 及建议 5 转场的范围优先，建议 6/extend 排除。审核本阶段源忠实度、完整性与适用性，不将公式变成固定段标题、词数、句数、面部 3–4 个细节或相同用词门禁。Creator 每 task 亲写及 use 同步另由最终输入审核核对，独立稿批量序列化不等于语义代写。
...
5. 视觉描述遵循共享 video prompt 规则，完整表达镜头动作、关键情绪/意图和摄影设计，并按实际 purpose 区分参考控制；细模可采用已有结构/材质或只借空间，不强制模仿动作。
...
核对 Creator 基线在各源 shot 的单行 `视频风格` 中表达并与配置相容。同组字段须精确相同，materials 提取单一源风格一次；最终 prompt 表达统一基线，可结尾重申必要全局要求。差异交 owner，局部变化留 prose，不模糊去重。详细动作、情绪意图、对白与音效留正文，use 说明实际用途和适用区间；不要求未来分组先完成才能审核摄影稿。

当前独立 Reviewer 先定位 shot/阶段的具体几何或故事冲突及影响，再列可选修法，侧面高角度、OTS、插入或 POV 均可。所有源 prose 与后续最终 prompt 无论参考有无肢翼或是否演细节，都须依源完整保留相关序列、必要主体/部位归属、准备/执行/收尾和接触变化，不编动作或设每帧配额。SVD 粗模只制作整体站位/轨迹、摄影/运镜、空间、光照、转场和时钟，不制作手臂、手掌、翅膀或精细机构动作；机构简化属 SVD 边界。已有粗动画优先省去信号复用，不省文字；建议 7 对带肢翼另强调模仿风险，不是完整性条件。缺手不判悬浮/unknown，不补代理或细 rig；独立 action reference 按所选动作审。具体源矛盾须修，不穷举潜在行为；不要求未来媒体先存在。

Prose 始终写清谁做什么、朝向/左右、归属、持握/接触、换手、机构动作及必要初中末、先后/重叠、触发反应与结果；参考有无肢翼不改变完整性。后续 `use`/prompt 声明素材到任务区间及维度，每次显式视频引用就地给源视频区间、维度和输出任务时间，同钟全组亦适用。带肢翼另查模仿风险。静态图无播放时钟；静态 shape/identity、精细几何及细模结构/材质按 purpose 保留，也不免最终源语义完整，不降 BOX 或重生身份图。源文字不依赖未来参考补全。

按 camera-language 的源意图判断每个切点：同一连续事件核对必要动作进度、持有/接触、空间与声音的相容延续，不要求相同帧；场/幕或时空跳转判断因果、情绪、信息、主题反差或平行关系与观众定位，不强制同位置、续动作、连续声音或过桥场。源支持的悬念、突兀感与硬切不必顺滑或立即解释；同集底层身份与世界事实一致，有意变化须有源依据。硬切可在动作中发生，终态可仍在行动/发声。TASK 不等于场景或幕，约束内可含多镜/多场，不强制幕结构、停步、结束姿势、停顿或下一组建立/重启。

独立 TASK 边界默认强烈优先有动机、明显不同的机位／视点／景别，降低近似构图独立生成差异的显眼程度。实际需要的匹配／重复构图及关键接触／必须无缝续声同组可有据保留，取舍不新增许可。不设每镜变化或角度配额，不保证连续性或豁免违背源意图的身份／状态／动作错接；重复构图本身不失败。保留源时长、连续成员、模型最大值和 grants；源重设计交 Director/owner 在原始预算内同步，不静默重组或强制叠化，未来装组未定不阻塞当前摄影审核。
...
按 [转场过程表达](../_meta/rules/transition-craft.md#转场过程表达) 核对切前末态、触发、过程与切后初态及必要方向/景别/声音。淡黑后显现是先后，交叉叠化是同窗重叠；硬切在源切点瞬时完成，不需新增过渡动画。源 clock、切点及预算保持，示例秒数不作配额。上传参考的内部轨迹线、坐标、camera cone 与调试标签属于后续清洁输入责任，正式片中文字仍保留；当前审核不提前要求未来参考。

按 [转场与片中文字](../_meta/rules/transition-craft.md) 核对成片文字的源依据、准确原词、可读窗口、对比、注意竞争及揭示顺序。场内 UI/纸面另查角色阅读/操作面，观众 SUPER、时间地点/章节卡、全屏卡不要求人物可见或可用。独立卡计镜头数及预算，叠字不重复计时，转场图像也不在装组额外加秒。内部源标题仅作为非成片元数据清理，有意上屏文字保留；不因文字、黑底或简单卡片失败，不将源设计验收当成片模型准确性保证。
...
以当前阶段材料判断观众能看到、听到什么；最终输入另对照实际 prompt/refs。非成片年份、罗盘方位与制作预算留在元数据，正式时间地点字幕/卡片的原词及作用保留。观众确需识别的事实由负责人选择源支持的视听证据；直接描述当前状态及注意变化，保持实际站位与持有关系。
