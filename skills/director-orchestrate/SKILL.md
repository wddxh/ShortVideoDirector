...
按 [参考用途与精细度](../_meta/rules/visual-prompt-craft-common.md#参考用途与精细度) 交接两项责任：Creator 最终 prompt 无论参考有无肢翼、是否演细节，始终按官方表达要求依源完整写相关动作序列及必要主体/部位归属、准备/执行/收尾、接触变化，不编动作或设每帧配额。SVD 粗模仅做整体移动/站位、camera/取景/运镜、转场、light、空间/reveals 和 clock，不制作手臂、手掌、翅膀或精细机构动作；机构简化属 SVD 边界。已有粗动画优先省去信号后复用，只省本地表现、不省文字；保留肢翼按建议 7 另处理模仿风险，不是完整性条件。细模/静态 shape/identity 与独立 action reference 按 purpose 保留，也不免源语义完整，不重生身份图或默认细 rig。缺手不判悬浮/unknown；具体位置/路径/机位、揭示、UI 时钟或可见源矛盾须修媒体，不穷举潜在行为。
...
交接剧本时说明原始预算、保留事实/原句和可开发范围，区分事件链与观众理解链。Scriptwriter 负责必要身份、关系、处境、欲望、利害、规则及关键行动/结果/反应；原创可在范围内发展缺失支撑，采用/修复需守住既有事实，实质冲突由 Director 协调。Storyboarder 负责取景、视点、切点与阅读窗口，Creator 在最终 authored prompt 完整保留必要事实、动作、关键文字与时序，参考按实际采用维度承载相应控制；源事实缺口交 owner，关键动作的文字保留要求不转成手部或关节代理制作要求。
...
任务视频集成委托纯文本独立 shot-input owner：先 start 再读最终 prompt/timeline/manifest/必要源，按用途规划相干窗口、切点、阅读与外部配对。所有最终 prompt 均核对完整源动作序列，无论参考有无肢翼/是否演细节；helper 另查粗动画信号、模仿风险和具体矛盾，独立 action reference 另查所选动作证据。整集覆盖全部相邻组，局部取必要邻界/故事依赖；fresh helper 比较选中 clean 的相干尾/头窗口与两端 prompt。owner 不加载图片、帧、contact sheet 或附件；查看交 fresh Reviewer 最小 helper 缩略图集。单资产叶子可有限查看写自身审核。scope 指定唯一 owner 的 canonical/STATE/payload 与 helper 临时预览；helper 不写同目标记录、不开轮次、不发 pass。
