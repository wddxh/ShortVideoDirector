...
- Claude Code 中使用 CronCreate/CronList/CronDelete 管理定时任务。
- Codex 中使用可用的 automation 能力管理定时任务；若当前环境没有 automation 能力，则提示用户周期性执行 `/check-video {目标} --auto`。
- Agent 表示使用可用的 sub-agent 隔离检查任务；若当前环境没有 sub-agent 能力，则在当前会话执行同等检查，但仍不得绕过 `check-video` skill。
...
1. 使用 CronList 或当前运行时等价的 automation 列表能力列出所有定时任务
...
为避免重活积压到主会话上下文，首次检查也用 Agent/sub-agent 机制起一个 general-purpose sub-agent 执行。sub-agent 的唯一职责是调用 check-video skill 并返回输出，主会话只处理返回的 JSON 摘要。

1. 使用 Agent/sub-agent 机制发起调用：
...
1. 使用 CronCreate 或当前运行时等价的 automation 创建定时任务：
   - `cron`: `*/{分钟数} * * * *`（如 20 分钟 → `*/20 * * * *`）
   - `recurring`: true
   - `prompt`: 见下方
...
1. 使用 Agent/sub-agent 机制发起一个 general-purpose sub-agent，prompt 为：
...
3. 按 JSON（或推断结果）决定：
   - all_complete == true → 使用 CronList 或等价能力找到本定时任务 → 使用 CronDelete 或等价能力删除自己；输出摘要 + human_needed 详情 + "可用 /check-video {目标} 手动处理"提示
   - 含 error 字段且 recoverable == false → 使用 CronList 或等价能力找到本定时任务 → 使用 CronDelete 或等价能力删除自己（防止反复空转浪费资源）；输出 error 描述给用户
...
- 不要绕过 Agent/sub-agent → check-video skill 路径直接执行查询/重试/生成
- 不要在主会话（cron/automation 触发会话）里直接调 check-video skill——必须通过 Agent/sub-agent 隔离上下文
```
