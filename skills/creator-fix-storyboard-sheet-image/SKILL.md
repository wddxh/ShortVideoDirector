...
2. 对每个选中 shot 用 Bash 删除整张 `assets/images/storyboard-sheets/{ep}/shotNN.png`，旧图不存在也继续。
3. 一次 Task 调用未来接口 `creator-generate-images {ep} paths {cards...}`，cards 使用完整路径；禁止逐 panel 生图。
4. 依据接口结果和实际 PNG 落盘情况，只把真实成功的 shots 计入成功集合。失败项保持失败，不声称已更新，不由本 skill enqueue impact。
