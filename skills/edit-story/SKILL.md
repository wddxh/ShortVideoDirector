...
所有 Task/relay 传 config_path；每次 Bash 显式用 `SVD_CONFIG="{config_path}"`。detect-mode 传 `"{config_path}"`，read-config 传键名后的该路径，check-episode 传 ep 后的该路径；fingerprint 仅使用 canonical config_path 与材料路径。videoProfile 和 evidence 必须使用同一 SVD_CONFIG，不依赖跨工具环境持久化。
...
provider/参数问题交真实 Creator Task 诊断当前能力与接入限制，主 AI 处理用户决定。固定共享/独立 sheet 值继续约束，缺值不授予选择权；任务选择不得改项目默认。路径仅定位材料，不授权 force。保留实际 grant、pending/receipt 设置，已有任务先按记录取回，不用当前配置重新选择。
...
请 Director 依据真实改动评估影响，不自动级联全部文件。Creator 的同步与生成能力保留编号/孤儿清理、pending 保护、force 目标和实际成功集合；不把删除的 card 传入生成路径。只重生确认受影响的图片，跨 sheet 连续性按直接依赖评估。旧 PNG 和历史 pass 都不能证明当前兼容，审核范围还须覆盖未决目标与缺证据的现有图片，不只看本次生成成功项。

返回实际改动、保留理由、当前审核证据、任务/pending 和未决决策。局部委托完成不等于整集就绪；整集交付运行 `SVD_CONFIG="{config_path}" bash "${CLAUDE_PLUGIN_ROOT}/scripts/check-episode.sh" "{ep}" "{config_path}"`。非零保持阻塞，exit 2 报 legacy；缺图、有效作用域提供方 none 或资源耗尽不豁免必需材料。
