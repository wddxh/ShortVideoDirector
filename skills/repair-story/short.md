...
- storyboard recovery：`storyboard:missing|incomplete|invalid` 时使用 Skill tool 调用 `storyboarder-storyboard` skill，参数 `ep01`；再使用 Skill tool 调用 `director-review-storyboard` skill，参数 `ep01`。Needs_revision 时使用 Skill tool 调用 `storyboarder-fix-storyboard` skill，参数 `ep01`；再使用 Skill tool 调用 `director-review-storyboard` skill，参数 `ep01`，共享 `fix_attempts=2`。通过后使用 Skill tool 调用 `creator-storyboard-sheet-prompts` skill，参数 `ep01 full`；保存 generator summary，再使用 Skill tool 调用 `director-review-storyboard-sheet-prompts` skill，参数 `ep01`。
- prompt owner loop：读取 `orchestrator handoff`，共享 `fix_attempts=2`。按序执行：`upstream-storyboard` 使用 Skill tool 调用 `storyboarder-fix-storyboard` skill，参数 `ep01`，再使用 Skill tool 调用 `director-review-storyboard` skill，参数 `ep01`；`generator` 使用 Skill tool 调用 `creator-storyboard-sheet-prompts` skill，参数 `ep01 incremental {shots...}`，集合变化时改为 `ep01 full`；`prompt-fix` 使用 Skill tool 调用 `creator-fix-storyboard-sheet-prompt` skill，参数 `ep01`；最后使用 Skill tool 调用 `director-review-storyboard-sheet-prompts` skill，参数 `ep01`。
- storyboard repair sheet rebuild：把 storyboard recovery 的 full generator 结果保存为 `saved_full_generator_summary`，并合并 prompt owner loop 的 `owner generator summaries`；prompt review 通过后解析其 `mode/created/updated/deleted`。把仍存在的 `created + updated` 映射为 `existing_changed_card_paths`，先使用 Skill tool 调用 `creator-generate-images` skill，参数 `ep01 paths {existing_changed_card_paths...}`，以 paths force 替换旧 PNG；再使用 Skill tool 调用 `creator-generate-images` skill，参数 `ep01 storyboard-sheets`，reconcile orphan 并补缺。合并两次实际 `successful shots union` 为 `successful_shots_union`，仅非空时使用 Skill tool 调用 `director-review-storyboard-sheets-visual` skill，参数 `ep01 {successful_shots_union...}`。显式 scope 不得被旧 PNG 或历史 pass 短路；`deleted cards never enter paths`。
