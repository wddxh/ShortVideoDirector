...
6. storyboard recovery：Storyboard 缺失或不完整时使用 Skill tool 调用 `storyboarder-storyboard` skill。再使用 Skill tool 调用 `director-review-storyboard` skill。
7. sheet.md 缺失、不 canonical 或 metadata 不符时使用 Skill tool 调用 `creator-storyboard-sheet-prompts` skill，参数 `{ep} full`。再使用 Skill tool 调用 `director-review-storyboard-sheet-prompts` skill；按 owner 顺序修复，最多 2 轮。
