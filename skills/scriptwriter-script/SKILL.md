...
## 输入
从委托确认目标集数、单集/系列语境、素材路径、保留要求与可调整范围。可直接采用成熟剧本，不必先有 outline / arc，也不要求内部参数表；范围不清先询问。
...
- outline / arc 仅在存在且相关时选读；用户提供的小说、章节和节选按委托的实际路径及采用范围读取，直接改编为剧本，源文保持原样
...
### Phase 4: 生成剧本
- 按 rules.md 建立理解当前行动所需的身份、关系、处境、欲望、利害与世界规则，分别组织事件链和观众理解链。原创在 scope 内发展缺失支撑；采用/修复依实际材料与授权，关键事实缺口交 Director 协调。
- 重要场景写清具体处境如何影响人物的表达与回应，给动作与反应留可表演的空间。有明确交涉目标时可用“试图达成什么、怎样尝试、对方怎样回应、局面怎样变化”组织，不将情绪反应一律解释为策略。薄场景可先想象具体现场再写对白，声音主导场景也可从听觉切入；rules.md 提供目标、利害关系、戏剧节拍与潜台词的解释和例子。
- 用人物生活与关系决定词汇、句法和称呼，让下一句受上一次回应影响。信息可随交涉显露，主题可由选择及后果承载；听者确有了解需要时可直接讲解，人物也可从自身处境直接讨论观点，不为套方法制造讨价还价。组织观众先看/听什么、何时明白，保留有作用的环境细节与声音主次，而非统一警句腔或加形容词。
- 新写时按场景级 schema 展开；采用现成剧本时保留合适的场景、对白和结局，仅补齐所需格式、路径、时长及清单，不无理由重写故事
- 由粗剧情与原始预算判断容量，暂定场景长短；完整对白、行动、结果和反应展开后再估时修正。关键成片事实及必要原字留在 script，取景/切点/阅读窗口交 Storyboarder，不先锁定比例或均分秒数。
...
### Phase 6: 自检
- 通读全篇的观众理解与转场：关键选择之前依据是否足够，谜团是否有意，时空跳转、因果和情绪余波能否跟随；线索可见不等于含义已懂。必要桥接可由现有场景承担，不强加过渡场。
- 按角色实际节奏试读关键互动并估算倾听/反应空间；仅文本推演如实标为估读。比较人物措辞而不只比较音色，核对观众证据与人物知情时点；VO、直接对白和沉默按具体作用判断，不按术语、字数或技巧数量验收。
- 保存后按 rules.md 实跑 `scene-duration.sh` 核对确认边界；`script-budget.sh` 仅作密度诊断，记录 status 与 `scenes_warn` / `scenes_missing` 等计数，汇总优先 `missing` > `warn` > `ok`；缺输入可无汇总，exit 0 不等于验收。密度警告不作创作 pass/fail，时长 gates 不变。
- 运行 `node ${CLAUDE_PLUGIN_ROOT}/scripts/episode-assets.mjs story/episodes/{ep}/script.md all` 验证清单格式；再逐场核对真实使用，解析成功不代表语义完整。
- schema、时长或身份校验失败在授权内修正；无法满足则报告阻塞。计数偏差需结合可拍性判断，不自动扩写。
