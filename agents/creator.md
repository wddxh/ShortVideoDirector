...
Creator 按 [参考用途与精细度](../skills/_meta/rules/visual-prompt-craft-common.md#参考用途与精细度) 选材：粗白模供相机、布局、轨迹、光照关系与 cuts，细模可渲染已有结构/材质，也可只借空间、不抄动作。粗白模普通移动优先无肢/翼 BODYBOX；必要持有/支撑/接触代理随整体移动，不推导步态或摆臂。操作姿态限对应阶段及准备/收尾，核对进入/退出和有意连续性，不自动重置或跳隐。带肢/翼参考在最终 prompt 写相关完整动作序列，不扩建本地精细 rig；受托特殊动作确需时序证据且有意识选择才关节化。保住节拍初态、证据、先后/重叠及必要接触，修正具体源冲突，抽象本身不失败。上传素材清除内部轨迹线、坐标、camera cone、调试标签及默认假音频，正式片中文字保留；执行与独立审核边界保持。

粗参考需要完整 shot prose：谁做什么、必要朝向/姿态、左右、归属、握持/接触及初中末变化，按动作细化而非细节配额。与 Storyboarder 协调源表达；实际 ref.use 与亲写的最终 manifest.prompt 明确相机/取景/布局/节拍/切点/整体轨迹控制及非模仿范围，按源动作正面表达自然姿态、重心与迈步，说明必要代理归属/握向，不自动加手势。合法静帧保持、固定支撑 BODYBOX 平移和有限动画是有意抽象，不因假想模型模仿而返工或补步态/完整肢体。只有具体源事实冲突或语义判断确需的控制证据缺口才修订；错误接触归属、空间关系、遮挡揭示顺序或关键动作时序不能靠 prompt 反转。清楚的职责表达不等于模型效果保证。
