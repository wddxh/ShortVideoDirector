...
description: 对单集短视频的任意内容（资产、大纲、剧本、分镜）提出修改意见。通过对话协商确定方案后，按级联 DAG 按需执行修正。使用 /short-edit-story 加自然语言描述触发。
user-invocable: true
...
This is a generated Codex wrapper. The source skill remains the single source of truth at `skills/short-edit-story/SKILL.md`.
...
1. Read `skills/short-edit-story/SKILL.md` and execute that skill's instructions with the user's original arguments.
2. Treat `skills/short-edit-story/` as the source skill directory. When the source skill references sibling files such as `rules.md` or `config-template.md`, resolve them relative to that directory.
