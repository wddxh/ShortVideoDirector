...
1. 读取 `${CLAUDE_PLUGIN_ROOT}/skills/creator-fix-storyboard-sheet-image/SKILL.md`，并使用用户的原始参数执行该 skill 的说明。
2. 将 `${CLAUDE_PLUGIN_ROOT}/skills/creator-fix-storyboard-sheet-image/` 视为源 skill 目录。当源 skill 引用 `rules.md` 或 `config-template.md` 等同级文件时，相对该目录解析。
