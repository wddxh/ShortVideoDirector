...
<!-- BEGIN ORIGINAL SKILL: skills/short-fix-outline/SKILL.md -->
...
根据修改意见（$ARGUMENTS[1]），定向修正现有单集短视频大纲中的具体问题。

## 规则

1. **只修改指出的问题** — 逐条对照 $ARGUMENTS[1] 中的修改意见进行修正，不得擅自修改未提及的内容
2. **严格遵循 rules.md** — 修正后的内容必须严格遵循 `skills/short-outline/rules.md` 中的所有格式和规则
