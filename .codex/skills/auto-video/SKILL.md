...
This is a generated Codex wrapper. The source skill remains the single source of truth at `skills/auto-video/SKILL.md`.
...
1. Read `skills/auto-video/SKILL.md` and execute that skill's instructions with the user's original arguments.
2. Treat `skills/auto-video/` as the source skill directory. When the source skill references sibling files such as `rules.md` or `config-template.md`, resolve them relative to that directory.
