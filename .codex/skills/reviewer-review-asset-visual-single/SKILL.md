...
这是生成的 Codex 适配层。源 skill 仍是唯一事实来源，位置为 `${CLAUDE_PLUGIN_ROOT}/skills/reviewer-review-asset-visual-single/SKILL.md`。
...
按 `skills/_meta/rules/visual-prompt-craft-common.md#全画面粗代理映射`：Creator 读实际参考，最终 prompt 开头覆盖所有可辨识粗对象/类别，含环境构件、地面墙背景、家具物品及无资产图对象。写实际颜色/形状/位置辨识→最终对象→采用空间包络（位置、屏占尺度、距离、朝向、路径）→源定最终形状结构、软硬、材质表面/光响应及源动作所需变化。同类重复可成组，不漏视觉类别、不设每网格配额或扩到无关不出场物。有上传图显式绑定，无图按已定源场景美术正面写外观，不强制新增图/卡；必要设计缺回 owner，不捏造资产/token。
...
1. 读取 `${CLAUDE_PLUGIN_ROOT}/skills/reviewer-review-asset-visual-single/SKILL.md`；入口保留用户原始自然语言请求，内部 skill 使用当前委托，不构造位置参数。
2. 将 `${CLAUDE_PLUGIN_ROOT}/skills/reviewer-review-asset-visual-single/` 视为源 skill 目录。当源 skill 引用 `rules.md`、`config-template.md` 或 provider 同级指南时，相对该目录解析。
