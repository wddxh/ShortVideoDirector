...
This is a generated Codex wrapper. The source skill remains the single source of truth at `skills/director-fix-outline/SKILL.md`.
...
1. Read `skills/director-fix-outline/SKILL.md` and execute that skill's instructions with the user's original arguments.
2. Treat `skills/director-fix-outline/` as the source skill directory. When the source skill references sibling files such as `rules.md` or `config-template.md`, resolve them relative to that directory.
